...
* global paths on WB computer
	...
	* start with individual data
	datalibweb, country(`countrycode') year(`year') type(GLDRAW) surveyid(PAK_2018_LFS_V01_M) filename(LFS 2018-19.dta)
	*   localpath(${rootdatalib}) local
}
...
if ("`c(username)'"=="sunquat") {
	* start with individual data
	use "${surveydata}/LFS 2018-19_sav  uploading", clear
}
...
rename S04Psn1 pid
* note: pid missing for 68 1-person HHs, so fill with pid = 1
recode pid (.=1)
...
g lstatus = .
replace lstatus = 1 if S05C02 == 1 // worked for pay or profit
replace lstatus = 1 if S05C03 == 1 // worked for family business 
replace lstatus = 1 if inlist(S05C04,1,2) // did not work but has a job/business
replace lstatus = . if S05C06 == 2 // absent more than 1 month

replace lstatus = 2 if S05C04 == 3 // planning to taking a job
replace lstatus = 2 if S09C01 == 1 & inlist(S09C04,1,2,3,4,5,6) // looked for work during past week and available
replace lstatus = 2 if inlist(S09C06,2,3,4,8)  // awaiting job OR temporarily laid off is apprentice OR property owner
...
* Note: Fix one observation where S07C031 and S07C041 are both answered for the same person.
recode S07C031 (0=.) if S07C041>0 & ~mi(S07C041)
...
* Note: Fix one observation where S07C3 and S07C4 are both answered for the same person.
recode S07C03 (0=.) if S07C04>0 & ~mi(S07C04)
