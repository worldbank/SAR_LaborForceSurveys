...
* global paths on WB computer
	...
	* start with individual data
	datalibweb, country(`countrycode') year(`year') survey(`survey') type(SARRAW) filename(2019Annual-Outfile-with-Computer.dta) localpath(${rootdatalib}) local
}
...
if ("`c(username)'"=="sunquat") {
	* load data
	* start with HH data
	use "${surveydata}/2019Annual-Outfile-with-Computer", clear
}
...
note educat4: For LKA_2019_LFS, to be consistent with LKA_2016_HIES we categorized "Special Education learning / learnt" as educat4 = 4 "Tertiary (complete or incomplete)".
...
note internet_athome: For LKA_2019_LFS, we used "internet use anywhere" as a proxy for "internet availability at home".
...
note contract: For LKA_2019_LFS, this question was only asked to employees.
...
g whours = q20
note whours: For LKA_2019_LFS, we used "number of hours you usually worked at this occupation work per week" instead of "number of hours you actually worked at this occupation during the reference period".
...
* note: fix one individual whose secondary job status is "employee" (q27 = 1), but who said they had income from salary (q46_a_1 = 2000, expected) and from being an employer or own account worker (q46_c_1 = 2000, likely accidentally duplicated amount recorded in q46_a_1).
replace q46_c_1 = . if hhid=="43038041" & pid==4
...
g whours_2 = q38
note whours_2: For LKA_2019_LFS, we used "number of hours you usually worked at this occupation work per week" instead of "number of hours you actually worked at this occupation during the reference period".
