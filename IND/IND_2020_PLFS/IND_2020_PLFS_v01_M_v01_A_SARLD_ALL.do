...
* global paths on WB computer
	...
	* NOTE: TODO: You (WB staff) will need to define the location of the datalibweb folder path here.
		tempfile hhvisit1
	datalibweb, country(`countrycode') year(`year') survey(`survey') type(SARRAW) filename(hhvisit1_2020-21.dta) localpath(${rootdatalib}) local
	drop state district nssregion stratum substratum subsample no_qtr
	save `hhvisit1', replace
	* start with individual data
	datalibweb, country(`countrycode') year(`year') survey(`survey') type(SARRAW) filename(personvisit1_2020-21.dta) localpath(${rootdatalib}) local
	* merge in HH data
...
if ("`c(username)'"=="sunquat") {
	* load data and merge
	* start with HH data
	use "${surveydata}/hhvisit1_2020-21", clear
	drop state district nssregion stratum substratum subsample no_qtr
	* merge with individual data
	merge 1:m hhid using "${surveydata}/personvisit1_2020-21", nogen assert(match)
}
...
g		int_year = 2020 if inlist(quarter,"Q5","Q6")
...
replace int_month=9 if  mi(int_month) & quarter=="Q5"
...
note relationtohead: For IND_2020_PLFS, the survey variable relationtohead = 7 "father/mother/father-in-law/mother-in-law" overlapped with two harmonized categories and was coded as 4 "parents".
note relationtohead: For IND_2020_PLFS, the survey variable relationtohead = 9 "servants/employees/other non-relatives" overlapped with two harmonized categories and was coded as 7 "non household members (domestic worker, room renter)".
...
note literacy: For IND_2020_PLFS, we used the general education variable that specified literate or non-literate, but didn't specify whether or not their definition of literacy included both reading and writing.
...
*********************
* labor
destring prn_occ nco_cws, replace force
destring sub_occupation nic_cws status*day? industry*day? *_industry duration_* *_day? earnings* *paid_leave ???_enterprise ???_job_contract ???_soc_security ???_n_workers	///
sub_status, replace
...
note contract_year: For IND_2020_PLFS, the source variable was only constructed for a subset of employed people.
...
note socialsec_year: For IND_2020_PLFS, we considered any of the following benefits as social security: pension, gratuity, or health.
...
* step 1: match by exact occupation
	* last week job1 occupation = 2nd occupation if they match and are non-missing
	g		lastweek_job1_12mojob = 2 if (nco_cws==sub_occupation) & ~missing(nco_cws)
	* last week job1 occupation = 1st occupation if they match and are non-missing
	replace lastweek_job1_12mojob = 1 if (nco_cws==prn_occ) & ~missing(nco_cws)
* step 2: match by status and aggregate occupation category
	* last week job1 status = 12-month job1 status AND last week job1 aggregate occupation category = 12-month job1 aggregate occupation category AND last week status and occupation are non-missing
	replace lastweek_job1_12mojob = 1 if missing(lastweek_job1_12mojob) & (status_cws==prn_status) & (floor(nco_cws/100)==floor(prn_occ/100)) & ~missing(status_cws) & ~missing(nco_cws)
	* last week job1 status = 12-month job2 status AND last week job1 aggregate occupation category = 12-month job2 aggregate occupation category AND last week status and occupation are non-missing
	replace lastweek_job1_12mojob = 2 if missing(lastweek_job1_12mojob) & (status_cws==sub_status) & (floor(nco_cws/100)==floor(sub_occupation/100)) & ~missing(status_cws) & ~missing(nco_cws)
...
* wage_nc = Wage payment, primary job, excl. bonuses, etc. (7-day ref period)
	...
	* from weekly recall workers in salaried jobs: last calendar month
	replace	wage_nc = earnings if inlist(status_cws,31,71,72) & lstatus==1 & empstat<=4
	* from weekly recall workers in self-employed jobs: 30 days
	replace wage_nc = earnings1 if inlist(status_cws,11,12,61,62) & lstatus==1 & empstat<=4
...
note unitwage: For IND_2020_PLFS, self-employed (status = 11 12 61 62) recall is actually 30 days, so coded as unitwage = 10 "Other" since there was not an exact match.
...
* wage_nc_week = Wage payment adjusted to 1 week, primary job, excl. bonuses, etc. (7-day ref period)
	...
	* from weekly recall workers in salaried jobs: last calendar month
	replace	wage_nc_week = earnings/4.3 if inlist(status_cws,31,71,72) & lstatus==1
	* from weekly recall workers in self-employed jobs: 30 days
	replace wage_nc_week = earnings1/30*7 if inlist(status_cws,11,12,61,62) & lstatus==1
...
note wage_nc_2: For IND_2020_PLFS, we could only create this variable for those in "casual labor".
...
note unitwage_2: For IND_2020_PLFS, we could only create this variable for those in "casual labor".
...
note wage_nc_week_2: For IND_2020_PLFS, we could only create this variable for those in "casual labor".
...
note contract: For IND_2020_PLFS, we were only able to create this for a subset of employed individuals.
...
note socialsec: For IND_2020_PLFS, we considered any of the following benefits as social security: pension, gratuity, or health.
