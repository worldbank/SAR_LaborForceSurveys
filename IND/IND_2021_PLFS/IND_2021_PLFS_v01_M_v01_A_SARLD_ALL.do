...
if ("`c(username)'"=="sunquat") {
	* load data and merge
	* start with individual data
	use "${surveydata}/personvisit1_2021-22", clear
	drop district stratum substratum subsample _merge
	* merge in HH data
	merge m:1 hhid using "${surveydata}/hhvisit1_2021-22", nogen assert(match)
}
...
g		int_year = 2021 if inlist(quarter,"Q3","Q4")
replace	int_year = 2022 if inlist(quarter,"Q1","Q2")
