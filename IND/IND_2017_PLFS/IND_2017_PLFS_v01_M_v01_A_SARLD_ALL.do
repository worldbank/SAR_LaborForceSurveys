...
* global paths on WB computer
	...
	* NOTE: TODO: You (WB staff) will need to define the location of the datalibweb folder path here.
		tempfile hhvisit1
	datalibweb, country(`countrycode') year(`year') survey(`survey') type(SARRAW) filename(hhvisit1_2017-18.dta) localpath(${rootdatalib}) local
	drop state district nssregion stratum substratum subsample no_qtr
	save `hhvisit1', replace
	* start with individual data
	datalibweb, country(`countrycode') year(`year') survey(`survey') type(SARRAW) filename(personvisit1_2017-18.dta) localpath(${rootdatalib}) local
	* merge in HH data
...
if ("`c(username)'"=="sunquat") {
	* load data and merge
	* start with HH data
	use "${surveydata}/hhvisit1_2017-18", clear
	drop state district nssregion stratum substratum subsample no_qtr
	* merge with individual data
	merge 1:m hhid using "${surveydata}/personvisit1_2017-18", nogen assert(match)
}
...
g		int_year = 2017 if inlist(quarter,"Q1","Q2")
...
note relationharm: For IND_2017_PLFS, the survey variable relationtohead = 7 "father/mother/father-in-law/mother-in-law" overlapped with two harmonized categories and was coded as 4 "parents".
note relationharm: For IND_2017_PLFS, the survey variable relationtohead = 9 "servants/employees/other non-relatives" overlapped with two harmonized categories and was coded as 7 "non household members (domestic worker, room renter)".
...
note literacy: For IND_2017_PLFS, we used the general education variable that specified literate or non-literate, but didn't specify whether or not their definition of literacy included both reading and writing.
...
* match week-recall job2 with 12-month recall job: 1 "1st job - from section 5.1", 2 "2nd job - from section 5.2"
* step 1 of 1: match by status and aggregate (10-category) sindustry
	* 12-month job1 not matched to last week job1 AND last week job2 status = 12-month job1 status AND last week job2 industry = 12-month job1 industry AND last week status and industry are non-missing
		* create week recall industry aggregated to industrycat10 categories
		recode industry_job2_week (1/3=1) (4/9=2) (10/33=3) (35/39=4) (41/43=5) (45/47 55/56=6) (49/53 58/63 79=7) (64/68 83=8) (84=9) (69/78 80/82 85/99=10) (*=.), g(industrycat10_job2_week)
	g		lastweek_job2_12mojob = 1 if lastweek_job1_12mojob~=1 & status_job2_week==status_prn & industrycat10_job2_week==industrycat10_year & ~missing(status_job2_week) & ~missing(industrycat10_job2_week)
	* 12-month job2 not matched to last week job1 AND last week job2 status = 12-month job2 status AND last week job2 industry = 12-month job2 industry AND last week status and industry are non-missing
	replace	lastweek_job2_12mojob = 2 if missing(lastweek_job2_12mojob) & lastweek_job1_12mojob~=2 & status_job2_week==status_sbs & industrycat10_job2_week==industrycat10_year & ~missing(status_job2_week) & ~missing(industrycat10_job2_week)
...
g		ocusec_2 = ocusec_year if lastweek_job2_12mojob==1 & lstatus==1
replace	ocusec_2 = ocusec_2_year if lastweek_job2_12mojob==2 & lstatus==1
...
g		paid_leave_2 = paid_leave_year if lastweek_job2_12mojob==1 & lstatus==1
replace	paid_leave_2 = paid_leave_2_year if lastweek_job2_12mojob==2 & lstatus==1
...
g		pensions_2 = pensions_year if lastweek_job2_12mojob==1 & lstatus==1
replace pensions_2 = pensions_2_year if lastweek_job2_12mojob==2 & lstatus==1
