...
* global paths on WB computer
	...
	* NOTE: TODO: You (WB staff) will need to define the location of the datalibweb folder path here.
		tempfile hhvisit1
	datalibweb, country(`countrycode') year(`year') survey(`survey') type(SARRAW) filename(hhvisit1_2019-20.dta) localpath(${rootdatalib}) local
	drop state district nssregion stratum substratum subsample no_qtr
	save `hhvisit1', replace
	* start with individual data
	datalibweb, country(`countrycode') year(`year') survey(`survey') type(SARRAW) filename(personvisit1_2019-20.dta) localpath(${rootdatalib}) local
	* merge in HH data
...
if ("`c(username)'"=="sunquat") {
	* load data and merge
	* start with HH data
	use "${surveydata}/hhvisit1_2019-20", clear
	drop state district nssregion stratum substratum subsample no_qtr
	* merge with individual data
	merge 1:m hhid using "${surveydata}/personvisit1_2019-20", nogen assert(match)
}
...
g		int_year = 2018 if inlist(quarter,"Q3","Q4")
...
note relationharm: For IND_2019_PLFS, the survey variable relationtohead = 7 "father/mother/father-in-law/mother-in-law" overlapped with two harmonized categories and was coded as 4 "parents".
note relationharm: For IND_2019_PLFS, the survey variable relationtohead = 9 "servants/employees/other non-relatives" overlapped with two harmonized categories and was coded as 7 "non household members (domestic worker, room renter)".
...
g hhmember = (inrange(relationharm,1,6))

* hsize = Household size, not including household workers
egen hsize = total(hhmember), by(hhid)
note hsize: This variable is defined as individuals whose relation with the household is known to be part of the household. This does not include relationharm = 7 "non household members (domestic worker, room renter)" or relationharm = missing, both of which occured in IND_2019_PLFS.
...
note literacy: For IND_2019_PLFS, we used the general education variable that specified literate or non-literate, but didn't specify whether or not their definition of literacy included both reading and writing.
