...
gen byte lstatus = .
replace lstatus = 1 if s4_1 == 1 // did at least 1h econ activity 
replace lstatus = 1 if s4_2 == 1 // absent
replace lstatus = 1 if emp == 1 

replace lstatus = 2 if inlist(s5_1, 1, 2) & s5_5 == 1  // looked for job/business in past 4 weeks & ready to start within the survey week
replace lstatus = 2 if inlist(s5_3, 1, 2) // awaiting job/business
...
g wage_nc_week = wage_nc if lstatus==1 & ~mi(wage_nc)

* wage_total - Annualized total wage, primary job (7-day ref period)
g wage_total = wage_nc * 4.3 * 12 if lstatus==1 & ~mi(wage_nc)
